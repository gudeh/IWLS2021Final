...
void AND::replaceInput(int index, nodeAig* param, bool param_polarity){
    if(param_polarity)
        param=((nodeAig*)(((uintptr_t)param) ^ 01));
    this->inputs[index]=param;
}
...
void AND::invertInputPolarity(int index){
    this->inputs[index]=this->inputs[index]->forceInvert();
}