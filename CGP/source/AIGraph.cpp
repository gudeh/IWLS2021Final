...
AND* AIGraph::updateAnd(unsigned int index, AND AND_obj){
//    cout<<"AND push, index:"<<index<<
    this->all_ANDS[index]=AND_obj;
//    this->all_ANDS[index].printNode();
    return &this->all_ANDS[index];
}

void AIGraph::printHeader(){
    cout<<"Header print: "<<this->name<<endl;
...
//        this->printCircuit();
        and_index++;
//        cout<<"CREATING AND "<<and_index<<"-----------------------------------"<<endl;
...
//            cout<<"extra:"<<AND_extra.getId()<<", iterator:"<<AND_extra.getId()/2-all_inputs.size()+1<<endl;
            AND_ptr=this->updateAnd((AND_extra.getId()/2)-all_inputs.size()+1,AND_extra);
...
//            AND_ptr->printNode();
        ...
        AND_ptr=this->updateAnd((AND_obj.getId()/2)-all_inputs.size()+1,AND_obj);
...
//    for(int y=0;y<O;y++)
//        this->all_outputs.push_back(output());
////    cout<<"CGP to AIG. \n AIG num of POs:"<<this->all_outputs.size()<<endl;
...
//            AND_ptr=this->updateAnd(and_index-I,AND_obj);
...
//    cout<<"cgp_vector size:"<<cgp_vector->size()<<endl;
//    int my_aux=0;
//    for(int t=cgp_vector->size()-1;t>cgp_vector->size()-20;t--)
//        cout<<cgp_vector->at(t)<<",";   cout<<endl;
//    for(int f=0;f<O;f++){
////        cout<<"f:"<<f<<endl;
//        my_aux=f*3;
...
//        PO_id=((cgp_vector->at(cgp_vector->size()-3-my_aux)+1)*(cgp_vector->at(cgp_vector->size()-2-my_aux))+(this->all_inputs.size()-1))*2;
//        
//        output output_obj(PO_id);
////        PO_polarity=cgp_vector->back();
//        PO_polarity=cgp_vector->at(cgp_vector->size()-1-my_aux);
...
void AIGraph::propagateSignalsCIFAR(CIFAR* cifar){
    ...
    for(it_out=all_outputs.begin();it_out!=all_outputs.end();it_out++)
        it_out->setBitVector(0);
    for(int i=0;i<cifar->getNumImages();i+=BITS_PACKAGE_SIZE)
    {
        ...
        while(bitvec_count<cifar->getPiSize())
       {
            bits.reset();
            for(int u=0;u<BITS_PACKAGE_SIZE;u++)
            {
//                cout<<"u+i"<<u+i<<","; 
                if(u+i<cifar->getImages()->size())
                    bits.set(u,cifar->getImages()->at(u+i).at(bitvec_count));}
            //cout<<endl;
//            cout<<cifar->getImages()->at(u+i).at(bitvec_count); cout<<"-";}
...
//            cout<<bits[0];
        ...
        if(it_in!=all_inputs.end())
            cout<<"WRONG1!!!!"<<it_in->getId()<<"!="<<all_inputs.end()->getId()<<endl;
//        cout<<bits.to_string()<<"->";
        ...
        for(it_out=all_outputs.begin();it_out!=all_outputs.end();it_out++)
        {
            if(it_out->getInputPolarity())
                it_out->setBitVector(~it_out->getBitVector());
            cout<<"outs:"<<bitset<BITS_PACKAGE_SIZE>(it_out->getBitVector())<<endl;
        }
        vector<unsigned long long int> POs_results;
         
         for(it_out=all_outputs.begin();it_out!=all_outputs.end();it_out++)
             POs_results.push_back(it_out->getBitVector());
        bitset<10> my_out;
        
        vector<unsigned long> package_outs;
        for(int u=i;u<i+BITS_PACKAGE_SIZE;u++)
        {
            my_out.reset();
//            for (int y=0;y<POs_results.size();y++)
            for (int y=POs_results.size()-1;y>=0;y--)
            {
                unsigned int bit=(POs_results[y] & ( 1 << u-i )) >> u-i;
                my_out.set(y,bit);
//                bool bit=cifar->getLabels()->at(u).at(y);
            }
            if(u<=cifar->getLabelInt()->size()-1)
            {
                if(cifar->getLabelInt()->at(u) == my_out.to_ulong())
                    correct_count++;
            }
//            package_outs.push_back(my_out.to_ulong()); //uth answer out of 10 POs
            cout<<"CORRECT ANSWERS:"<<correct_count<<endl;
        }
//        if(package_outs.size()!=BITS_PACKAGE_SIZE)
//            cout<<"SOMETHING WRONG!!! package size not expected:"<<package_outs.size()<<" != "<<BITS_PACKAGE_SIZE<<endl;
        
        
//            bits.reset();   
//            for(int u=i;u<i+BITS_PACKAGE_SIZE;u++)
//                bits.set(u-i,(bool)cifar->getLabelInt()->at(u));
//    //        cout<<bits[u];} cout<<"<-right answers"<<endl;
//            aux=~(it_out->getBitVector() ^ bits.to_ullong());
//            while(aux){   count+= aux & 1;    aux >>= 1;  }
//            if(it_out->getBitVector()==0)            constant0++;
//            if((~it_out->getBitVector())==0)            constant1++;
//            aux=it_out->getBitVector();
...
void AIGraph::setScore(float score){
    this->current_score=score;
}

void AIGraph::setSize(int size){
    this->functional_and_count=size;
}


nodeAig* AIGraph::findAny(unsigned int param) {
//    cout<<"looking for:"<<param<<endl;
//    cout<<"(this->all_inputs.size())*2):"<<((this->all_inputs.size())*2)<<endl;
...
//        all_ANDS[param/2-(all_inputs.size()-1)].printNode();
//        cout<<"index:"<<param/2-(all_inputs.size()-1)<<", size"<<all_ANDS.size()<<endl;
...
void AIGraph::mutateOffpsring(float mutation_chance,mt19937& mt){
    uniform_real_distribution<double> dist(0,100);
    uniform_int_distribution<int> dist_func(1,3);
    vector<AND>::iterator it_and;
    int fanin_mut=0,pol_mut=0,func_mut=0,new_id_first=0,new_id_second=0;
    float random=0;
    for(it_and=this->all_ANDS.begin()+1;it_and!=this->all_ANDS.end();it_and++)
    {
//        uniform_real_distribution<double> dist_new_node(1,(it_and->getId()-2)/2);
        uniform_int_distribution<int> dist_new_node(1,(it_and->getId()-2)/2);
        //chance to mutate first fanin 
        random=dist(mt);
        if(random<=mutation_chance)
        {
            ...
            fanin_mut++;
            #endif
            new_id_first=dist_new_node(mt)*2;
            it_and->replaceInput(0,findAny(new_id_first),it_and->getInputPolarities()[0]);
        }
        ...
        if(random<=mutation_chance)
        {
            ...
            pol_mut++;
            #endif
            it_and->invertInputPolarity(0);
        }
        //chance to mutate second fanin
        random=dist(mt);
        if(random<=mutation_chance)
        {
            #if COUT >= 1
            fanin_mut++;
            #endif
            new_id_second=dist_new_node(mt)*2;
            while(new_id_second==new_id_first)
                new_id_second=dist_new_node(mt)*2;
            it_and->replaceInput(1,findAny(new_id_second),it_and->getInputPolarities()[1]);
        }
        //chance to mutate second fanin polarity
        random=dist(mt);
        if(random<=mutation_chance)
        {
            #if COUT >= 1
            pol_mut++;
            #endif
            it_and->invertInputPolarity(1);
        }
        it_and->invertInputs();
    }
#if COUT >= 1
    cout<<"CGP-> mutation chance:"<<mutation_chance<<"%, fanins:"<<fanin_mut<<", polarities:"<<pol_mut<<", functions:"<<func_mut<<endl;
...
void AIGraph::copyAIG(AIGraph* in_aig,mt19937& mt){
    this->name=in_aig->getName();
    vector<AND>::iterator it_and;        vector<output>::iterator it_out;       vector<input>::iterator it_in;
    int A=in_aig->getAnds()->size()-1,I=in_aig->getPIs()->size()-1,O=in_aig->getPOs()->size();
    for(int y=0;y<=I;y++)
        this->all_inputs.push_back(input());
    for(int y=0;y<O;y++)
        this->all_outputs.push_back(output());
    ...
    /////////////ANDS/////////////////////////
    int and_index=I,PI_size=all_inputs.size();
    bool polar=false;
    AND* AND_ptr;
    for(int l=0;l<A;l++){
        and_index++;
        unsigned int AND_id=and_index*2;
        AND AND_obj(AND_id);
        AND_ptr=this->updateAnd((AND_id/2)-PI_size+1,AND_obj);        
        AND_ptr->pushInput(this->findAny(in_aig->findAny(AND_id)->getInputs()[0]->getId()),
                            (bool)in_aig->findAny(AND_id)->getInputPolarities()[0]);
        AND_ptr->pushInput(this->findAny(in_aig->findAny(AND_id)->getInputs()[1]->getId()),
                    (bool)in_aig->findAny(AND_id)->getInputPolarities()[1]);
    }
    ////////////////////OUTPUTS///////////////////
    for(int f=0;f<O;f++){
        int PO_id=in_aig->getPOs()->at(f).getId();
        output output_obj(PO_id); 
        output_obj.pushInput(this->findAny(PO_id),in_aig->getPOs()->at(f).getInputPolarity());
        this->pushPO(f,output_obj);
    }
}